    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            ...
            R.id.action_bookmark -> {
                if (isBookmarked) {item.setIcon(R.drawable.ic_bookmark_border_white); isBookmarked = false}
                else {item.setIcon(R.drawable.ic_bookmark_fill_white); isBookmarked = true}
                bookmarkArticle(
                ArticleData(
